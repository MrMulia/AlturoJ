...
public class LoginAPI {
    ...
    public Response login(String bodyJSON, @Context HttpServletRequest request) throws JSONException {
        ...
        // User enumeration vulnerability: differentiating between non-existing user and wrong password
        // This will show in the error reponse
        try {
            if (!DBUtil.isValidUser(username, password)) {
                boolean userExists = DBUtil.userExists(username); // created new method 'userExists' in DBUtil
                if (!userExists) {
                    myJson.put("error", "Username not found: " + username);
                } else {
                    myJson.put("error", "Incorrect password for user: " + username);
